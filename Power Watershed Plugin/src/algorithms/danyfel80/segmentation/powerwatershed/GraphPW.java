...
public class GraphPW {
  ...
  private int[]             normalWeights;               // edge normalized
                                                         // weights
   ...
   *          Amount of edges
   * @param seq
   *          Sequence where the segmentation is performed
   * @param seeds
   *          Seed ROIS. One ROI per label.
   */
  public GraphPW(Sequence seq, List<ROI> seeds, boolean useGeo,
      boolean useGrayLevels, boolean useQuickSort) {
  ...
  public void SetupSeeds() {
    ...
    for (int i = 0; i < numLabels; i++) {
      ...
      System.out.printf(
          "color %d for " + nameLabels[i] + " = " + colorLabels[i] + "\n", i);
  ...
  public void calculateWeights() {
    ...
    for (i = 0; i < M; i++) {
      ...
      for (j = 0; j < channels; j++) {
        val = Math.abs(TypeUtil.unsign(seqData[z1][j][xy1])
            - TypeUtil.unsign(seqData[z2][j][xy2]));
        if (j == 0) {
          weights[i] = val;
        } else {
          weights[i] = Math.max(weights[i], val);
        }
      }
    ...
    for (i = 0; i < M; i++) {
      // normalWeights[i] = 255 - (int)
      // Math.round(((double)normalWeights[i]/(double)maxVal) * 255.0);
    ...
    // if not dilated the result of segmentation is voronoi because weights are
    // all 0.
    DilateReconstruction.reconstruct(seedsFunction, normalWeights, weights,
        edges, sx, sy, sz, M, useQuickSort);
  }
      ...
      for (y = 0; y < sy; y++) {
        for (x = 0; x < sx; x++) {
          ...
          for (k = 1; k <= numNeighbors; k++) {
            n = GraphUtils.getNeighborNodeEdge(x + y * sx + z * sy * sx, k, sx,
                sy, sz);
            if (n != -1) {
              w += normalWeights[n];
              l++;
   ...
   * 
   * @param useGeo
   *          Make geodesic reconstruction.
   */
  public void executeWatershed() {
    if (useGeo) {
      seqResult = executePWQ2(edges, weights, weights, 255, indexSeeds,
          indexLabels, numSeeds, sx, sy, sz, numLabels);
    } else {
      seqResult = executePWQ2(edges, weights, normalWeights, 255, indexSeeds,
          indexLabels, numSeeds, sx, sy, sz, numLabels);
    }
  }

  /**
   * Computes the result sequence of the energy minimization : min_x lim_p->inf
   * sum_e_of_E w_{ij}^p |x_i-x_j|^2
   * 
   * @param edges
   *          Array of node indexes composing edges
   * @param weights
   ...
   * @param indexSeeds
   *          Array of seeded nodes' indexes
   * @param indexLabels
   *          Label values on the seeded nodes
   * @param numSeeds
   *          Amount of seeded nodes
   * @param sx
   *          X-coordinate size of the sequence
   * @param sy
   *          Y-coordinate size of the sequence
   * @param sz
   *          Z-coordinate size of the sequence
   * @param numLabels
    ...
    while (edgeCount < M) {
      ...
      while (!plateauStack.isEmpty()) {
        ...
        for (k = 1; k <= numNeighborEdges; k++) {
          ...
          y = ((sz > 1)? GraphUtils.getNeighborEdge3D(e1, e2, x, k, sx, sy, sz)
      ...
      if (numEdges > 0) {
        ...
        if (differentSeeds) {
          ...
          if (useQuickSort) {
            GraphUtils.binSortDec(sortedWeights, sortedNormalEdges, numEdges,
                maxWeight + 1);
          } else {
            GraphUtils.quickStochasticSortDec(sortedWeights, sortedNormalEdges,
                0, numEdges - 1);
          }
          ...
          numNormalEdges = 0;
          for (normalEdgeCount =
              0; normalEdgeCount < numEdges; normalEdgeCount++) {
          ...
          if (numVertices < MAX_SIZE_PLATEAU) {
            success = RandomWalker.ExecuteRandomWalker(edgesLCP, numNormalEdges,
                plateauVerticesList, plateauValueIndicators, numVertices,
                localSeeds, localLabels, k, numLabels, proba);
          }

          if (numVertices >= MAX_SIZE_PLATEAU || !success) {
            System.out.printf(
                "Plateau of a big size (%d vertices, %d edges) the RW is not performed on it.\n",
   ...
   *          Index of node 2
   * @param ranks
   *          Array needed for union-find efficiency
   * @param fathers
   *          Array for storing roots of merged nodes trees
   * @param proba
   *          Array for storing the result x
   * @param numLabels
   *          Amount of labels
   */
  ...
  public Collection<ROI3DArea> getSegmentationROIs() {
    ...
    for (int z = 0; z < sz; z++) {
      for (int y = 0; y < sy; y++) {
        for (int x = 0; x < sx; x++) {
          rois.get((seqResData[z][x + y * sx] + 1) % colorLabels.length)
              .addPoint(x, y, z);
        }
      }
    }
