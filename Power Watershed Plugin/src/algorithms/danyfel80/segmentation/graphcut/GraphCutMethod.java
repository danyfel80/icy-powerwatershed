...
public class GraphCutMethod {
	 ...
	 * graph.isInChangedList(node, false); graph.setTimestamp(node, time);
	 * 
	 * if (graph.getResidualNodeCapacity(node) > 0) { // node is connected to
	...
	while ((node1 = queueStart) != GraphGC.NONE) {
		...
		if (graph.getParent(node1) == GraphGC.NONE || graph.isInSink(node1)) {
			...
			if (!graph.isMarked(node2)) {
				...
				if (graph.getParent(node2) != GraphGC.NONE && graph.isInSink(node2)
	...
	for (int orphanEdge = graph.getFirstOutgoing(orphan); orphanEdge != GraphGC.NONE; orphanEdge = graph
	    .getNextEdge(orphanEdge))
		...
		if (!graph.isInSink(node) && parentEdge != GraphGC.NONE) {
			...
			for (node = graph.getHead(orphanEdge); graph.getTimestamp(node) != time; node = graph
			    .getHead(graph.getParent(node))) {
