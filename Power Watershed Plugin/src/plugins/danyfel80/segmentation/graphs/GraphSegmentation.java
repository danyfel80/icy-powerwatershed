 ...
 * 
 * This plugin implements the segmentation framework proposed in the article
 * "Power Watershed: A Unifying Graph-Based Optimization Framework" by Camille
 * Couprie, Leo Grady, Laurent Najman, Hugues Talbot.
 * 
...
public class GraphSegmentation extends EzPlug {
  ...
  protected void initialize() {
    ...
    inShowProbas.setToolTipText(
        "If checked the gradient and terminal probabilities sequences are created.");;
    ...
    inUseGrayLevels
        .setToolTipText("If true sequence is converted to gray levels.");
    inUseGeodesicReconstruction =
        new EzVarBoolean("Use geodesic reconstruction.", false);
    inUseGeodesicReconstruction.setToolTipText(
        "If true the weight is normalized using a geodesic reconstruction.");
    optional.add(inUseGrayLevels);
    optional.add(inUseGeodesicReconstruction);

    inSegmentationType
        .addVarChangeListener(new EzVarListener<SegmentationType>() {

          @Override
          public void variableChanged(EzVar<SegmentationType> source,
              SegmentationType newValue) {
            for (@SuppressWarnings("rawtypes")
            EzVar option: optional) {
    ...
    try {
      ...
      case PowerWatershedQ2:
        ...
        PowerWatershedSegmentation a6 = new PowerWatershedSegmentation(
            inSequence.getValue(),
            inSeedsSequence.getValue().getROIs(ROI.class),
            inUseGeodesicReconstruction.getValue(), inUseGrayLevels.getValue());
        endTime = System.nanoTime();
        algo = a6;
      ...
      MessageDialog.showDialog(String.format(
          "Power Watershed Plugin failed for the algorithm \"%s\"!", algo));
      e.printStackTrace();
    }

    System.out.println(
        "Time to prepare algorithm: " + (endTime - startTime) / 1000000 + "ms");
