  ...
  protected void execute() {
    ...
    try {
      ...
      case GraphCuts:
        ...
        algo = new GraphCutSegmentation(inSequence.getValue(), lambda, inUse8Connected.getValue(),inSeedsSequence.getValue().getROIs(ROI2D.class));
      ...
      case PowerWatershedQ2:
        startTime = System.nanoTime();
        algo = new PowerWatershedSegmentation(inSequence.getValue(), inSeedsSequence.getValue().getROIs(ROI2D.class), false/*TODO make user visible*/);
        endTime = System.nanoTime();
        break;
    ...
    if (inSeedsSequence.isEnabled()) {
      if (inSeedsSequence.getValue() == null) {
        ...
        if (inSeedsSequence.getValue().getROIs().isEmpty()) {
          ...
          if (inP.getValue() == 1 && inQ.getValue() == 1 && rois.size() != 2) {
            MessageDialog.showDialog("Error", 
                "Please select a sequence with ROI's specifying the seeds " +
