...
public class GraphCutSegmentation extends SegmentationAlgorithm {
	 ...
	 */
	public GraphCutSegmentation(Sequence sequence, List<ROI> seeds, float lambda, double edgeVariance,
	    boolean use8Connected, boolean createProbas) throws BadHistogramParameters {
	inSequence = sequence;
	this.seeds = seeds;
	this.lambda = lambda;
	...
	int[] currVals = new int[sc], neighVals = new int[sc], neighVals1 = new int[sc];
	...
	int sxy = sx * sy;
	numN = sxy * sz;
	numE = (use8Connected) ? sz * (13 * sxy - 9 * sx - 9 * sy + 6) - sy * (9 * sx - 6) + 6 * sx - 4
		...
		for (x = 0; x < sx; x++) {
			...
			for (dy = 0; dy < 2 && dy + y < sy; dy++) {
				...
				if (dx + dy + dz > 0) {
					...
					weight = (float) getEdgeLikelihood(currVals, neighVals, dx, dy, dz);
					...
					if (dx + dy == 2) {
						...
						for (c = 0; c < sc; c++) {
						neighVals[c] = TypeUtil.unsign(seqData[z][c][y * sx + (x + dx)]);
						neighVals1[c] = TypeUtil.unsign(seqData[z][c][(y + dy) * sx + x]);
						}
						weight = (float) getEdgeLikelihood(neighVals, neighVals1, dx, dy, dz);
					...
					if (dx + dz == 2) {
						...
						for (c = 0; c < sc; c++) {
						neighVals[c] = TypeUtil.unsign(seqData[z][c][y * sx + (x + dx)]);
						neighVals1[c] = TypeUtil.unsign(seqData[z + dz][c][y * sx + x]);
						}
						weight = (float) getEdgeLikelihood(neighVals, neighVals1, dx, dy, dz);
						graphCut.setEdgeWeight(neighPos, neighPos1, weight);
						// Sets weight on edges in both directions
					...
					if (dy + dz == 2) {
						...
						neighPos1 = (z + dz) * sxy + y * sx + x;
						for (c = 0; c < sc; c++) {
						neighVals[c] = TypeUtil.unsign(seqData[z][c][(y + dy) * sx + x]);
						neighVals1[c] = TypeUtil.unsign(seqData[z + dz][c][y * sx + x]);
						}
						weight = (float) getEdgeLikelihood(neighVals, neighVals1, dx, dy, dz);
						graphCut.setEdgeWeight(neighPos, neighPos1, weight);
					...
					if (dx + dy + dz == 3) {
						...
						neighPos1 = (z + dz) * sxy + y * sx + x;
						for (c = 0; c < sc; c++) {
						neighVals[c] = TypeUtil.unsign(seqData[z][c][(y + dy) * sx + (x + dx)]);
						neighVals1[c] = TypeUtil.unsign(seqData[z + dz][c][y * sx + x]);
						}
						weight = (float) getEdgeLikelihood(neighVals, neighVals1, dx, dy, dz);
						graphCut.setEdgeWeight(neighPos, neighPos1, weight);
						...
						neighPos1 = z * sxy + (y + dy) * sx + x;
						for (c = 0; c < sc; c++) {
						neighVals[c] = TypeUtil.unsign(seqData[z + dx][c][y * sx + (x + dx)]);
						neighVals1[c] = TypeUtil.unsign(seqData[z][c][(y + dy) * sx + x]);
						}
						weight = (float) getEdgeLikelihood(neighVals, neighVals1, dx, dy, dz);
						graphCut.setEdgeWeight(neighPos, neighPos1, weight);
						...
						for (c = 0; c < sc; c++) {
						neighVals[c] = TypeUtil.unsign(seqData[z + dz][c][(y + dy) * sx + x]);
						neighVals1[c] = TypeUtil.unsign(seqData[z][c][y * sx + (x + dx)]);
						}
						weight = (float) getEdgeLikelihood(neighVals, neighVals1, dx, dy, dz);
						graphCut.setEdgeWeight(neighPos, neighPos1, weight);
						// Sets weight on edges in both directions
	...
	for (i = 0; i < 2; i++) {
		...
		if (createProbas)
			terminalData[sDI.getPositionZ()][sDI.getPositionY() * sx + sDI.getPositionX()] = val;
		sDI.next();
		}
	}
	if (createProbas) {
	...
	double val = Math.exp(-(diff * diff) / (2.0 * edgeVariance)) / Math.sqrt(dx * dx + dy * dy + dz * dz);
	return val;
	}
	...
	if (resultSegmentation == null) {
		...
		resultSegmentation.setName(inSequence.getName() + "(var" + edgeVariance + ", lambda" + lambda + ", "
	...
	result.setName(inSequence.getName() + "Segmentation(var" + edgeVariance + ", lambda" + lambda + ", "
	    + ((use8Connected) ? "8" : "4") + "-conn)");
	return result;
	}
