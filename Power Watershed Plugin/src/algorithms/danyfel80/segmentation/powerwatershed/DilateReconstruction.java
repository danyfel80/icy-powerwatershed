...
public class DilateReconstruction {
  ...
  /**
   * Reconstruction by dilation of normalWeights under seedsFunction. Union-find
   * method described by Luc Vicent.
   * 
   * @param seedsFunction
   *          Image seeds
   ...
   *          Image weights
   * @param resultWeights
   *          Result of the reconstruction by dilation of normalWeights under
   *          seedsFunction.
   * @param edges
   *          List of couples of vertices forming edges
   * @param sx
   *          Size in X of sequence
   * @param sy
   *          Size in Y of sequence
   * @param sz
   *          Size in Z of sequence
   * @param M
   *          Amount of edges
   */
  public static void reconstruct(int[] seedsFunction, int[] originalWeights,
      int[] resultWeights, int[][] edges, int sx, int sy, int sz, int M,
    ...
    if (sz == 1) { // 2D image
      for (k = M - 1; k >= 0; k--) {
        ...
        for (i = 1; i <= 6; i++) { // parcourt les 6 voisins
          ...
          if (n != -1) {
            if (markedEdges[n]) {
              elementLinkGeodesicDilate(n, p, parentEdges, originalWeights,
                  resultWeights);
            }
          }
          markedEdges[p] = true;
      ...
      for (k = M - 1; k >= 0; k--) {
        ...
        for (i = 1; i <= 12; i++) { // parcourt les 12 voisins
          n = GraphUtils.getNeighborEdge3D(edges[0][p], edges[1][p], p, i, sx,
              sy, sz);
          if (n != -1)
            if (markedEdges[n])
              elementLinkGeodesicDilate(n, p, parentEdges, originalWeights,
  ...
  private static void elementLinkGeodesicDilate(int childedge, int parentEdge,
    ...
    if (reconstructedNode != parentEdge) {
      if (normalWeights[reconstructedNode] == normalWeights[parentEdge]
          || normalWeights[parentEdge] >= weights[reconstructedNode]) {
        fathers[reconstructedNode] = parentEdge;
        weights[parentEdge] =
            Math.max(weights[reconstructedNode], weights[parentEdge]);
      } else {
