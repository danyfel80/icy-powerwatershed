...
public class PowerWatershedSegmentation extends SegmentationAlgorithm {
  ...
  public PowerWatershedSegmentation(Sequence sequence, List<ROI> seeds,
      boolean useGeo, boolean useGrayLevels) throws BadHistogramParameters {
    this.useGeo = useGeo;
    inSequence = sequence;
  ...
  protected void prepareGraph(List<ROI> seeds) throws BadHistogramParameters {
    ...
    System.out.println(
        "seeds prepared: " + ((endTime - startTime) / 1000000) + " msec...");
    startTime = System.nanoTime();
    graph.SetupEdges();
    endTime = System.nanoTime();
    System.out.println(
        "edges prepared: " + ((endTime - startTime) / 1000000) + " msec...");
    startTime = System.nanoTime();
    graph.calculateWeights();
    endTime = System.nanoTime();
    System.out.println(
        "weights computed: " + ((endTime - startTime) / 1000000) + " msec...");
  }

  /*
   * (non-Javadoc)
   * @see
   * algorithms.danyfel80.segmentation.SegmentationAlgorithm#executeSegmentation
   * (java.util.List)
   */
  @Override
