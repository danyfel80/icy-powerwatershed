...
public class GraphPW {
	...
	for (i = 0; i < M; i++) {
		...
		val = Math.abs(TypeUtil.unsign(seqData[z1][j][xy1]) - TypeUtil.unsign(seqData[z2][j][xy2]));
		if (j == 0) {
			weights[i] = val;
		} else {
	...
	if (useGeo) {
		...
		seqResult = executePWQ2(edges, weights, normalWeights, 255, indexSeeds, indexLabels, numSeeds, sx, sy, sz,
		    numLabels);
	}
	}

	/**
	 * Computes the result sequence of the energy minimization : min_x
	 * lim_p->inf sum_e_of_E w_{ij}^p |x_i-x_j|^2
	 * 
	 * @param edges
	 ...
	 */
	private Sequence executePWQ2(int[][] edges, int[] weights, int[] normalWeights, int maxWeight, int[] indexSeeds,
	    int[] indexLabels, int numSeeds, int sx, int sy, int sz, int numLabels) {
	...
	while (edgeCount < M) {
		...
		if (differentSeeds) {
			...
			GraphUtils.quickStochasticSortDec(sortedWeights, sortedNormalEdges, 0, numEdges - 1);
			}
			...
			success = RandomWalker.ExecuteRandomWalker(edgesLCP, numNormalEdges, plateauVerticesList,
			    plateauValueIndicators, numVertices, localSeeds, localLabels, k, numLabels, proba);
			}
	...
	/**
	 * Updates the result(proba), ranks and fathers arrays when 2 nodes are
	 * merged
	 * 
	 * @param e1
	 *          Index of node 1
	 ...
	 *          Amount of labels
	 */
	private void mergeNode(int e1, int e2, int[] ranks, int[] fathers, float[][] proba, int numLabels) {
	int k, re1, re2;
	re1 = GraphUtils.findElement(e1, fathers);
	re2 = GraphUtils.findElement(e2, fathers);
