...
public class GraphCutMethod {
   ...
   *          counterpart (i.e., the directed edge in the other direction) count
   *          as one edge.
   */
  public GraphCutMethod(int numNodes, int numEdges) {
    System.out
        .println("Num nodes = " + numNodes + ", Num edges = " + (2 * numEdges));
  ...
  /**
   * Set the affinity for one node to belong to the foreground (i.e., source) or
   * background (i.e., sink).
   *
   * @param node
   ...
   */
  public float setTerminalWeights(int node, float source, float sink,
      float lambda, boolean update) {

    source = (source > maxTerminalWeight)? lambda * maxTerminalWeight
        : lambda * source;
    sink =
        (sink > maxTerminalWeight)? lambda * maxTerminalWeight: lambda * sink;
   ...
   *
   * Please note that you cannot call any <tt>setEdgeWeight</tt> more often than
   * the number of edges you specified at the time of construction!
   *
   * @param node1
   *          The first node.
   * @param node2
   ...
   *          node1.
   */
  public void setEdgeWeight(int node1, int node2, float weight1to2,
      float weight2to1) {

    // get edge indices
    int edge = edgeNum;
    edgeNum++;
    int reverseEdge = edgeNum;
    edgeNum++;
   ...
   * @param changedNodes
   *          list of nodes that potentially changed their segmentation compared
   *          to a previous call, can be set to <tt>null</tt>
   */
  public float computeMaximumFlow(boolean reuseTrees,
    ...
    while (true) {
      ...
      if (!graph.isInSink(activeNode)) {
        // grow source tree
        for (edge =
            graph.getFirstOutgoing(activeNode); edge != GraphGC.NONE; edge =
                graph.getNextEdge(edge)) {
          if (graph.getResidualEdgeCapacity(edge) != 0) {
            ...
            } else if (graph.getTimestamp(headNode) <= graph
                .getTimestamp(activeNode)
                && graph.getDistance(headNode) > graph
                    .getDistance(activeNode)) {
  ...
  private void maxflowInit() {
    ...
    int[] sourceNodes = new int[graph.getNumNodes()];
    int[] sinkNodes = new int[graph.getNumNodes()];
    int posSrc = 0, posSnk = 0;

    for (int node = 0; node < graph.getNumNodes(); node++) {
      if (graph.getResidualNodeCapacity(node) > 0) {
        // node is connected to source
        sourceNodes[posSrc++] = node;
      } else if (graph.getResidualNodeCapacity(node) < 0) {
        // node is connected to sink
        sinkNodes[posSnk++] = node;
      } else {
    ...
    int node;
    while (posSnk > 0 || posSrc > 0) {
      if (posSrc > 0) {
        posSrc--;
        node = sourceNodes[posSrc];
    ...
    /*
     * for (int node = 0; node < graph.getNumNodes(); node++) {
     * graph.setNextNode(node, GraphGC.NONE); graph.isMarked(node, false);
     * graph.isInChangedList(node, false); graph.setTimestamp(node, time); if
     * (graph.getResidualNodeCapacity(node) > 0) { // node is connected to
     * source graph.isInSink(node, false); graph.setParent(node,
     * GraphGC.TERMINAL); setNodeActive(node); graph.setDistance(node, 1); }
     * else if (graph.getResidualNodeCapacity(node) < 0) { // node is connected
     * to sink graph.isInSink(node, true); graph.setParent(node,
     * GraphGC.TERMINAL); setNodeActive(node); graph.setDistance(node, 1); }
     * else { graph.setParent(node, GraphGC.NONE); } }
     */
  }
    ...
    while ((node1 = queueStart) != GraphGC.NONE) {
      ...
      if (graph.getResidualNodeCapacity(node1) > 0) {
        ...
        if (graph.getParent(node1) == GraphGC.NONE || graph.isInSink(node1)) {
          ...
          for (edge =
              graph.getFirstOutgoing(node1); edge != GraphGC.NONE; edge =
                  graph.getNextEdge(edge)) {
            ...
            if (!graph.isMarked(node2)) {
              ...
              if (graph.getParent(node2) != GraphGC.NONE
                  ...
                  && graph.getResidualEdgeCapacity(edge) > 0)
                setNodeActive(node2);
            }
          }
  ...
  private void augment(int middle) {
    ...
    for (node = graph.getHead(graph.getSister(middle));; node =
        graph.getHead(edge)) {
    ...
    // 2a - the source tree
    graph.setResidualEdgeCapacity(graph.getSister(middle),
        graph.getResidualEdgeCapacity(graph.getSister(middle)) + bottleneck);
    graph.setResidualEdgeCapacity(middle,
        graph.getResidualEdgeCapacity(middle) - bottleneck);
    for (node = graph.getHead(graph.getSister(middle));; node =
        graph.getHead(edge)) {
      ...
      graph.setResidualEdgeCapacity(edge,
          graph.getResidualEdgeCapacity(edge) + bottleneck);
      graph.setResidualEdgeCapacity(graph.getSister(edge),
          graph.getResidualEdgeCapacity(graph.getSister(edge)) - bottleneck);
      if (graph.getResidualEdgeCapacity(graph.getSister(edge)) == 0)
        addOrphanAtFront(node);
    }
    graph.setResidualNodeCapacity(node,
        graph.getResidualNodeCapacity(node) - bottleneck);
    if (graph.getResidualNodeCapacity(node) == 0)
      addOrphanAtFront(node);
  ...
  private void processSourceOrphan(int orphan) {
    ...
    for (int orphanEdge =
        graph.getFirstOutgoing(orphan); orphanEdge != GraphGC.NONE; orphanEdge =
            graph.getNextEdge(orphanEdge))
      if (graph.getResidualEdgeCapacity(graph.getSister(orphanEdge)) != 0) {
        ...
        if (!graph.isInSink(node) && parentEdge != GraphGC.NONE) {
          ...
          if (distance < Integer.MAX_VALUE) { // node originates from the
                                              // source
            ...
            for (node = graph.getHead(orphanEdge); graph.getTimestamp(
                node) != time; node = graph.getHead(graph.getParent(node))) {
    ...
    if (bestEdge != GraphGC.NONE) {
      ...
      // process neighbors
      for (int orphanEdge = graph
          .getFirstOutgoing(orphan); orphanEdge != GraphGC.NONE; orphanEdge =
              graph.getNextEdge(orphanEdge)) {
        ...
        if (!graph.isInSink(node) && parentEdge != GraphGC.NONE) {
          ...
          if (graph.getResidualEdgeCapacity(graph.getSister(orphanEdge)) != 0)
            setNodeActive(node);
          if (parentEdge != GraphGC.TERMINAL && parentEdge != GraphGC.ORPHAN
              && graph.getHead(parentEdge) == orphan)
